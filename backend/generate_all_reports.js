...
const basicSyntaxTopics = ["Control Flow", "Loops", "Patterns", "Functions", "Recursion"];
...
// 2. LeetCode base list (95 titles)
...
async function main() {
  ...
  const resolveTopic = (subcategoryName, title) => {
    ...
    if (subLower.includes('sorting') || titleLower.includes('sorting') || titleLower.includes('sort')) return 'Sorting';
  ...
  striverA2ZData.forEach((category, catIdx) => {
    ...
    category.subcategories.forEach((sub, subIdx) => {
      ...
      sub.problems.forEach((prob, probIdx) => {
        ...
        // Check if already in inventory (internal duplicate check in source sheet)
  ...
  customFxDbTitles.forEach(title => {
    ...
    if (canonicalInventoryMap.has(normTitle)) {
      ...
      // Report if overlap between custom and striver/leetcode
      overlapReports.push({
        title: existing.title,
        sources: [...existing.sourcesList]
      });
  ...
  activeDbTitles.forEach(titleKey => {
    ...
    if (/\s+\d+$/.test(title) && !title.toLowerCase().includes('ii') && !title.toLowerCase().includes('iii') && !title.toLowerCase().includes('iv')) {
      ...
      if (canonicalInventoryMap.has(baseTitleNorm)) {
        duplicateClones.push({
          id: metadata.id,
          title: title,
          canonicalBase: canonicalInventoryMap.get(baseTitleNorm).title
        });
      }
    }
  });
  ...
  canonicalInventory.forEach(q => {
    ...
    if (!dbMatch) {
      ...
      // Let's check if there is a case-insensitive but not exact whitespace match, or casing mismatch
...
| :--- | :--- | :--- | :--- |
${overlapReports.map((o, idx) => `| ${idx + 1} | **${o.title}** | ${o.sources.join(' and ')} | Merged as **${o.sources[0]}** |`).join('\n')}
`;
